...
<html lang="en">
  <head>
    ...
    <title>quiz</title>
<!--     <link rel="stylesheet" href="../../css/exam/quiz.css" /> -->
    <link rel="stylesheet" href="../../css/exam/quizTestingQuestioins.css" />
...
ps.setInt(1, QuestionsDao.count());
ResultSet result = ps.executeQuery();

// Initialize variables
int correctOptionCount = 0;
int totalQuestions = QuestionsDao.count();
...
if (result.next()) {
    ...
    do {
        // Retrieve question data from the result set
...
<%
    ...
    // Redirect to totalmarks.jsp if all questions are answered
    if (totalQuestions == 0) {
        response.sendRedirect("totalmarks.jsp?marks=" + correctOptionCount);
    }
} else {
...
if (result.next()) {
    ...
    System.out.println(" in the questions   "+questionNumber);
    System.out.println(question+" "+option1ImagePath+" "+option2ImagePath+" "+option3ImagePath+" "+option4ImagePath+"\n");
...
<%
		...
		if (selectedOption != null && Integer.parseInt(selectedOption) == correctOption) {
   			 correctOptionCount++;
		}
        questionNumber++;
    } while (result.next() && totalq>-1);
...
<div class="right">
          <div class="top">
            ...
            <div style="display: flex;">
              <a  href="#" style="text-decoration: none; display: inline-block; position: relative; none; margin:5px">
          ...
          <div class="bottom " >
            ...
            <div class="coloring-btn" style="background-color: #bad3ed ; margin-top: -21px;">
           


       		<%
     con = QuestionsDao.getConnection();
     ps = con.prepareStatement("SELECT COUNT(*) as total FROM questions");
